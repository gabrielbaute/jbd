  ...
  <main class="container mx-auto max-w-5xl px-4 py-12 min-h-screen flex flex-col relative">
    ...
    <div class="fixed top-6 right-6 z-40">
      <button 
        ...
        class="p-3 bg-slate-900/80 border border-slate-800 rounded-2xl text-slate-500 hover:text-neon-green transition-all backdrop-blur-md shadow-2xl"
      >
    ...
    <!-- Pantalla de carga de metadata -->
    <section v-if="isAnalyzing" class="flex-1 flex items-center justify-center">
    ...
    <!-- Área de trabajo (Metadata cargada) -->
    <section v-if="albumData" class="space-y-8 animate-[fadeIn_0.5s_ease-out]">
      ...
      <div v-if="showProgressPanel" class="space-y-8 animate-slide-up">
          ...
          <div class="glass-card p-6 rounded-2xl border-b-4 border-neon-green bg-slate-900/40 shadow-2xl">
            <div class="flex justify-between items-end mb-4">
              <div>
                <h2 class="text-white font-black text-2xl uppercase italic tracking-tighter">
                  {{ isCompleted ? 'Download Ready' : 'Archivando Album...' }}
                </h2>
            ...
            <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
              <div 
                class="h-full bg-neon-green transition-all duration-500 shadow-[0_0_15px_#4ade80]"
