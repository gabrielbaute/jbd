...
export const useSettings = () => {
  ...
  /**
   * Procesa la carga del contenido de las cookies.
   */
  const saveCookies = async (): Promise<void> => {
    if (!cookieContent.value) return;
    try {
      savingCookies.value = true;
      await settingsService.uploadCookiesContent(cookieContent.value);
      message.value = { text: 'Cookies actualizadas con éxito.', type: 'success' };
    } catch (e) {
      message.value = { text: 'Error al escribir el archivo de cookies.', type: 'error' };
    } finally {
